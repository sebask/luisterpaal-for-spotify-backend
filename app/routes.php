...
Route::get('/update', function() // Update the cache behind the scenes, ideal for cronjobs ;)
{
	return 'Refreshed the cache with ' . count(get_luisterpaal_albums()) . ' albums!';
});

Route::get('/empty', function() // Clear the cache manually :)
{
	Cache::forget('albums');
...
function get_luisterpaal_albums()
{
	...
	// Get all the spotify albums for the luisterpaal albums
